...
- 把沙箱配置改成 `Mode=full` + `ProxyKind=socks5` 后**重启进程**；
- 两组单选各自**恰好一个**被选中，且就是配置里那个（**两个同时选中**是 Win32
  单选回填的经典坑，见下方坑 5）；
- 代理地址框 / 端口输入框 / 端口微调都跟着恢复出来的 SOCKS5 一起**启用**
  （回填失败时它们会保持禁用，出现"界面选了 SOCKS5、代理框却是灰的"）。
...
`shot-4-clone-back.png` / `shot-5-restore.png` 五张截图。
...
**现象**：网络代理选 SOCKS5（或克隆模式选整站）→ 关窗 → 下次启动，两个单选同时
被勾上；而且代理地址框还是灰的。

**根因**（两条实现细节叠加）：

- 单选的互斥是 **`BS_AUTORADIOBUTTON` 在响应用户点击时**才做的。程序化发
  `BM_SETCHECK` **不会**取消同组兄弟 —— 而 winsafe 的 `RadioButton::select(b)`
  恰好就是裸的 `BmSetCheck`。于是"构造时带了 `selected: true` 的那个 + 配置里恢复的
  那个"双双保持勾选。
- winsafe 的 `RadioGroup::selected_index()` 实现是
  `position(|r| r.is_selected())` —— 返回**第一个**被选中的。两个都勾着时它认出的是
  序号小的那个（直连 / 单页模板）。

两条一叠加，后果就不只是观感了：**界面显示 SOCKS5、实际按直连跑**，代理框还因为
`selected_index()` 读到"直连"而整条禁用；关窗回写时配置里的 `ProxyKind` 被静默改成
`none` —— 设置悄悄丢了。

**修法**：程序化设置单一一律走 `select_radio_only()`，把同组所有项显式设一遍
（选中的设 true、其余设 false），不要单独 `select(true)`。

> **测试口径**：这个 bug 前面 42 条断言一条都没抓到 —— 它们只覆盖"首次运行"这条路径。
> 单选回填属于**另一条独立路径**（配置文件里存着非默认值 → 回填控件），必须重启进程
> 才能走到。现在由阶段二（`gui_probe.py --restore`）覆盖：改沙箱配置 → 重启 → 断言
